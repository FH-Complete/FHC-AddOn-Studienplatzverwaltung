...
class aqa_foebis_person extends basis_db
{
	/** @var $new boolean */
	private $new = true;		
	/** @var DB-Result */
	 ...
	 */
	public function __construct($foebis_person_id=null)
	{
		parent::__construct();
		
		$this->datum_obj = new datum();
		if(!is_null($foebis_person_id))
			$this->load($foebis_person_id);
	}
	...
	/**
	 * Laedt einzelne Personendaten der ID $foebis_person_id	
	 * @param integer $foebis_person_id ID des zu ladenden Datensatzes
	 * @return boolean Description true wenn ok, false im Fehlerfall
	 */
	public function load_foebis($foebis_person_id	)
	{
		//Pruefen ob adress_id eine gueltige Zahl ist
		if(!is_numeric($foebis_person_id	) || $foebis_person_id	 == '')
		{
			$this->errormsg = 'foebis_person_id muss eine Zahl sein';
			return false;
		}

		//Daten aus der Datenbank lesen
		$qry = "SELECT * FROM addon.tbl_aqa_foebis_person WHERE foebis_person_id=".$this->db_add_param($foebis_person_id, FHC_INTEGER, false);
	...
	private function mapRow($target,$row) {		
		$target->foebis_person_id	= $row->foebis_person_id;
		...
		$target->foerderrelevant	= $this->foerderrelevant;
	...
	public function load_foebis_person($matrikelnr)
	{
		...
		if(!is_numeric($foebis_person_id) || $foebis_person_id == '')
		{
			$this->errormsg = "foebis_person_id muss eine gültige Zahl sein";
			return false;
		}
	...
	{
		if(!is_numeric($this->foebis_person_id) && $this->foebis_person_id!='')
		{
			$this->errormsg='foebis_person_id enthaelt ungueltige Zeichen';
			return false;
		}
	 ...
	 * andernfalls wird der Datensatz mit der ID in $foebis_person_id aktualisiert
	 * @return boolean true wenn ok, false im Fehlerfall
	 */
	public function save()
	{
		...
		if($this->new)
		{
			...
			$qry='BEGIN;INSERT INTO addon.tbl_aqa_foebis_person ('.
			     ...
			     'meldedatum,foerderrelevant,insertamum, insertvon, '.
		...
		{
			//Pruefen ob foebis_person_id eine gueltige Zahl ist
			if(!is_numeric($this->foebis_person_id))
			{
				$this->errormsg = 'foebis_person_id muss eine gueltige Zahl sein';
				return false;
			}
			$qry='UPDATE addon.tbl_aqa_foebis_person SET'.				
				...
				' foerderrelevant='.$this->db_add_param($this->foerderrelevant).', '.
		      	' updateamum= now(), '.
		      	' updatevon='.$this->db_add_param($this->updatevon).		      	
		      	' WHERE foebis_person_id='.$this->db_add_param($this->foebis_person_id, FHC_INTEGER, false).';';
		}
		...
		if($this->db_query($qry))
		{
			if($this->new)
			{
				...
				$qry="SELECT currval('addon.tbl_aqa_foebis_person_foebis_person_id_seq') as id;";
				if($this->db_query($qry))
				{
					if($row = $this->db_fetch_object())
					{
						$this->foebis_person_id = $row->id;
		...
		return $this->foebis_person_id;
	}

	
	public function deleteByStgKz($studiengang_kz)
	{		
		...
		$qry="DELETE FROM addon.tbl_aqa_foebis_person "
				. "WHERE "
	 ...
	 * @param $foebis_person_id ID die geloescht werden soll
	 * @return true wenn ok, false im Fehlerfall
	 */
	public function delete($foebis_person_id)
	{
		...
		//loeschen des Datensatzes
		$qry="DELETE FROM addon.tbl_aqa_foebis_person WHERE foebis_person_id=".$this->db_add_param($foebis_person_id, FHC_INTEGER, false).";";
