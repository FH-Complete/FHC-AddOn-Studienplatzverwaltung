...
angular.module('ngGrid.services').factory('$domUtilityService',['$utilityService', '$window', function($utils, $window) {
    ...
    domUtilityService.AssignGridContainers = function($scope, rootEl, grid) {
        ...
        // [WM]
        grid.$footerContainer = grid.$footerPanel.find(".ngFooterContainer");
        $scope.$footerContainer = grid.$footerContainer;
        
        grid.$footerScroller = grid.$footerPanel.find(".ngFooterScroller");
        grid.$footers = grid.$footerScroller.children();

        $scope.$watch(function () {
            return grid.$viewport.scrollLeft();
        }, function (newLeft) {
            grid.$footerContainer.scrollLeft(newLeft);  // [WM]
    ...
    domUtilityService.BuildStyles = function($scope, grid, digest) {
        ...
        css = "." + gridId + " .ngCanvas { width: " + trw + "px; }" +
            ...
            "." + gridId + " .ngFooterScroller { width: " + (trw + domUtilityService.ScrollH) + "px}";
...
var ngColumn = function (config, $scope, grid, domUtilityService, $templateCache, $utils) {
    ...
    self.footerCellTemplate = colDef.footerCellTemplate || $templateCache.get('footerCellTemplate.html'); // [WM]
    self.aggCellTemplate = colDef.aggCellTemplate || $templateCache.get('aggCellTemplate.html').replace(CUSTOM_FILTERS, self.cellFilter ? "|" + self.cellFilter : ""); // [WM]
    ...
    if (colDef.footerCellTemplate && !TEMPLATE_REGEXP.test(colDef.footerCellTemplate)) {
        self.footerCellTemplate = $.ajax({
            type: "GET",
            url: colDef.footerCellTemplate,
            async: false
        }).responseText;
    }
    if (colDef.aggCellTemplate && !TEMPLATE_REGEXP.test(colDef.aggCellTemplate)) {
        self.aggCellTemplate = $.ajax({
            type: "GET",
            url: colDef.aggCellTemplate,
            async: false
        }).responseText;
    }
    ...
    self.setVars = function (fromCol) {
        ...
        self.footerCellTemplate = fromCol.footerCellTemplate; //[WM]
        self.cellTemplate = fromCol.cellTemplate;
        self.aggCellTemplate = fromCol.aggCellTemplate;  //[WM]
...
var ngFooter = function ($scope, grid) {
    ...
    $scope.rows = function() {
        return grid.filteredRows.length > 0 ? grid.filteredRows : grid.rowCache;
    };  // [WM]    
...
var ngGrid = function ($scope, options, sortService, domUtilityService, $filter, $templateCache, $utils, $timeout, $parse, $http, $q) {
    ...
    self.$footerContainer = null;  // [WM]
    self.$footerScroller = null;   // [WM]
    self.$headers = null;
    self.$footers =  null;  // [WM]
    self.$viewport = null;
    self.$canvas = null;
    ...
    $scope.viewportDimHeight = function() {
        return Math.max(0, self.rootDim.outerHeight - $scope.topPanelHeight() - (2*$scope.footerRowHeight) - 2);  // [WM]
    };
...
var ngStyleProvider = function($scope, grid) {
	...
	$scope.footerScrollerStyle = function() {
        return { "height": grid.config.footerRowHeight + "px" };
    };
        ...
        compile: function() {
            return {
                pre: function($scope, iElement) {
                    ...
                    var aggCellTemplate = $scope.col.aggCellTemplate.replace(COL_FIELD, 'row.entity.' + $scope.col.field); //[WM]


                    if ($scope.col.enableCellEdit  && !$scope.$parent.row.isAggRow) {  //[WM]
                    ...
                    } else if ($scope.$parent.row.isAggRow) { //[WM]
                        html = aggCellTemplate;       //[WM]
                    } else {
                        html = cellTemplate;
                    }
...
ngGridDirectives.directive('ngFooterCell', ['$compile', function($compile) {
    var ngHeaderCell = {
        scope: false,
        compile: function() {
            return {
                pre: function($scope, iElement) {
                    iElement.append($compile($scope.col.footerCellTemplate)($scope));
                }
            };
        }
    };
...
ngGridDirectives.directive('ngViewport', [function() {
    return function($scope, elm) {
        ...
        elm.bind('scroll', function(evt) {
            ...
            if ($scope.$footerContainer) {
                $scope.$footerContainer.scrollLeft(scrollLeft);
            }
...
angular.module("ngGrid").run(["$templateCache", function($templateCache) {
  ...
  $templateCache.put("cellEditTemplate.html",
    "<div ng-cell-has-focus ng-dblclick=\"CELL_EDITABLE_CONDITION && editCell()\">\r" +   // [WM]
    "\n" +
  ...
  $templateCache.put("aggCellTemplate.html",
  ...
  $templateCache.put("gridTemplate.html",
    ...
    "<div ng-show=\"showFooter\" class=\"ngFooterPanel\" ng-class=\"{'ui-widget-content': jqueryUITheme, 'ui-corner-bottom': jqueryUITheme}\" ng-style=\"footerStyle()\">\r" +  // [WM]    
	"	<div class=\"ngFooterContainer\" ng-style=\"footerStyle()\">\r" +  // [WM]
			"<div ng-grid-footer />" +				
	"	</div>\r" +  // [WM]
	"	<div class=\"ngFooterTotalSelectionPanel\" ng-style=\"footerStyle()\">\r" + // [WM]
		
	"		<div class=\"ngTotalSelectContainer\" >\r" +		// [WM]
	"			 <div class=\"ngFooterTotalItems\" ng-class=\"{'ngNoMultiSelect': !multiSelect}\" >\r" +		// [WM]
	"				 <span class=\"ngLabel\">{{i18n.ngTotalItemsLabel}} {{maxRows()}}</span><span ng-show=\"filterText.length > 0\" class=\"ngLabel\">({{i18n.ngShowingItemsLabel}} {{totalFilteredItemsLength()}})</span>\r" +		// [WM]
	"	        </div>\r" +		// [WM]
	"		     <div class=\"ngFooterSelectedItems\" ng-show=\"multiSelect\">\r" +		// [WM]
	"			     <span class=\"ngLabel\">{{i18n.ngSelectedItemsLabel}} {{selectedItems.length}}</span>\r" +		// [WM]
	"			</div>\r" +		// [WM]
	"		</div>\r" +	// [WM]
  ...
  // [WM]
  $templateCache.put("footerCellTemplate.html",
    "<div class=\"ngHeaderSortColumn {{col.headerClass}}\" ng-style=\"{'cursor': col.cursor}\" ng-class=\"{ 'ngSorted': !noSortVisible }\">\r" +
    "\n" +
    //"    <div  ng-class=\"'colt' + col.index\" class=\"ngHeaderText\">{{col.displayName}}</div>\r" +
    "\n" +
    "</div>\r" +
    "\n" +
    "<div ng-show=\"col.resizable\" class=\"ngHeaderGrip\" ng-click=\"col.gripClick($event)\" ng-mousedown=\"col.gripOnMouseDown($event)\"></div>"
